...
class interfaz():
    ...
    def cuadriculas(self):
        ...
        self.canvas.create_line(0, segundo_diente_iz_y,
                                mitad_x, segundo_diente_iz_y)
        self.canvas.create_line(940, segundo_diente_de_y,
                                mitad_x, segundo_diente_de_y)
    ...
    def conf_imagen(self, apoyoa: Image, X, Y,  ancho=-1, alto=-1, rotacion=0, flip=False, extra: Image = None, separado=0, vertical=False, Ampliar_separacion=False, crop=(-1, -1, -1, -1), expand=False):
        ...
        medidas = (int(ancho*width*crecimiento/1920),
                   int(alto*height*crecimiento/1080))
        apoyoa = apoyoa.resize(medidas)
        self.x = int((X)*width/1920)
        self.y = int((Y)*height/1080)
        if(extra is not None):
            extra = extra.resize(medidas)
            apoyoa = self.get_concat_h_cut_center(
                apoyoa, extra, separacion=separado, ampliar=Ampliar_separacion)
        if (flip):
        ...
        if(self.cuadricula):
            if(x1 < mitad_x):
                ...
                self.canvas.create_line(
                    mitad_x, mitad_x*m+b, 2*mitad_x, (2*mitad_x)*m+b)
        if(pos_y < m*pos_x+b):
            return True
        else:
            return False
    ...
    def asignar(self, diente, x, y):
        ...
        if(self.cuadricula):
            self.canvas.create_line(
                x-dist, y, x+dist, y, width=5, fill="green")
            self.canvas.create_line(
                x, y-dist, x, y+dist, width=5, fill="green")
    ...
    def crearImagenBoton(self, imagen, imagen_teoria=None, titulo_teoria=None, mensaje=None, restrictivo=False):
        ...
        self.Botones[self.botonActual -
                     1].grid(column=self.botonActual, row=1, padx=5)
        self.botonActual += 1

    def crear_Borrador_(self):
        im1 = Image.open('./src/borrador.png')
        im2 = Image.open('./src/lapiz.png')
        im1 = im1.resize((var.size, var.size), Image.ANTIALIAS)
        im2 = im2.resize((var.size, var.size), Image.ANTIALIAS)
        self.Imagenes.append(itk.PhotoImage(im1))
        self.Botones.append(ttk.Button())
        self.Botones[self.botonActual-1] = ttk.Button(
        ...
        self.Imagenes.append(itk.PhotoImage(im2))
        self.Botones.append(ttk.Button())
        self.Botones[self.botonActual-1] = ttk.Button(
            self.frame, image=self.Imagenes[-1], command=lambda: self.permitir_dibujar())
        self.Botones[self.botonActual -
                     1].grid(column=self.botonActual+1, row=1, padx=5)
    ...
    def crear_teoria(self, imagen, titulo, mensaje):
        ...
        self.img = self.img.resize(
            (self.ancho, int(13*self.largo/64)), Image.ANTIALIAS)
        self.img = itk.PhotoImage(self.img)
        self.label = ttk.Label(self.frame, image=self.img).place(
            x=0, y=int(5*self.largo/8))
        self.tamaño = Font(family="Bahnschrift", size=int(width/100))
        self.fuente = Font(family="Roboto Mono", size=int(width/196))
        self.titulo = ttk.Label(self.frame, font=self.tamaño, text=titulo, width=self.ancho).place(
            x=19*self.ancho/64, y=55*self.largo/64)
    ...
    def permitir_dibujar(self):
        self.permitir_dibujo = True

    def paint(self, event):
        if (self.permitir_dibujo):
            paint_color = '#8f0b12'
            if self.old_x and self.old_y:
                self.canvas.create_line(
                    self.old_x, self.old_y, event.x, event.y, fill=paint_color, 
                    smooth=True, splinesteps=36, width=5, tag=self.tag, capstyle = "round",
                    joinstyle = "round")
            self.old_x = event.x
            self.old_y = event.y
